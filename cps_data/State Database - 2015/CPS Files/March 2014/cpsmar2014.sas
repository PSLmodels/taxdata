...
libname library "C:\Users\anderson.frailey\Documents\State Database - 2015\CPS Tax Units\2014\Extracts\";

*  The following line should contain
   the complete path and name of the raw data file.
   On a PC, use backslashes in paths as in C:\	 ;

FILENAME datafile "C:\Users\anderson.frailey\Documents\State Database - 2015\CPS Files\March 2014\asec2014_pubuse_tax_fix_5x8_2017.dat";

*  The following line should contain the name of the SAS dataset ;

%let dataset = cpsmar2014 ;
...
attrib  thprop_val   length=3     label="Topcode flag for HPROP-VAL";           
attrib  i_propval    length=3     label="Allocation flag for HPROP-VAL";        
...
attrib  tcare_val    length=3     label="Topcode flag for CARE-VAL";            
attrib  care_val     length=4     label="Annual amount paid for child";         
attrib  i_careval    length=3     label="Allocation flag for CARE-VAL";         
...
attrib  i_fhipval    length=3     label="Allocation flag for FHIP-VAL";         
...
attrib  semp_val     length=5     label="ERN-YN = 1 or SEOTR = 1";              
...
attrib  chelsew_yn   length=3     label="Does this person have a child living  ";
attrib  axrrp        length=3     label="Relationship to reference person";     
...
attrib  axlfsr       length=3     label="Labor force status recode allocation"; 
...
attrib  i_potcval    length=3     label="Imputation item:POTC-VAL";             
attrib  i_pmedval    length=3     label="Imputation item: PMED-VAL";            
attrib  i_chspval    length=3     label="Imputation item: CHSP-VAL";            
attrib  i_chspyn     length=3     label="Imputation item: CHSP-YN";             
attrib  i_chelsewyn  length=3     label="Imputation item: CHELSEW-YN";          
...
attrib  tphip_val    length=3     label="Health insurance premiums";            
attrib  tpotc_val    length=3     label="OTC medical expenses";                 
attrib  tpmed_val    length=3     label="Medical expenses (except OTC)";        
attrib  tchsp_val    length=3     label="Child support paid";                   
...
LABEL
	...
	thprop_val= "Topcode flag for HPROP-VAL"            
	i_propval = "Allocation flag for HPROP-VAL"         
	...
	tcare_val = "Topcode flag for CARE-VAL"             
	care_val  = "Annual amount paid for child"          
	i_careval = "Allocation flag for CARE-VAL"          
	...
	i_fhipval = "Allocation flag for FHIP-VAL"          
	...
	chelsew_yn= "Does this person have a child living  "
	axrrp     = "Relationship to reference person"      
	...
	axlfsr    = "Labor force status recode allocation"  
	...
	i_potcval = "Imputation item:POTC-VAL"              
	i_pmedval = "Imputation item: PMED-VAL"             
	i_chspval = "Imputation item: CHSP-VAL"             
	i_chspyn  = "Imputation item: CHSP-YN"              
	i_chelsewyn= "Imputation item: CHELSEW-YN"           
	...
	tphip_val = "Health insurance premiums"             
	tpotc_val = "OTC medical expenses"                  
	tpmed_val = "Medical expenses (except OTC)"         
	tchsp_val = "Child support paid"                    
; 
...
in a sas data set called fcpmar2014t .
To use the stored formats in a subsequent program,
include something like this:

proc format cntlin=library.fcpmar2014t;
PROC freq;
        tables PULAYCK1 ;
...
PROC FORMAT cntlout=library.fcpmar2014t;
...
VALUE i_phipval	(default=32)
	...
	1         =  " Allocated at family level (non-   eld"
	2         =  " Allocated at individual level (elderl"
	3         =  " Missing in family with at least one v"
	4         =  " Value changed to $0 because all famil"
	5         =  " Logical imputation equal to $0 for el"
;
...
proc print data=library.cpsmar2014t (obs=6);
...
FORMAT
	...
	tchsp_val  tchsp_val.
;
proc contents data=library.cpsmar2014t;
run;
/*
