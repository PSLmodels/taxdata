...
def adjfilst(cps_recs):
    """
    """
    cps_recs["case1"] = np.where(
        ((cps_recs["filst"] == 0) & (cps_recs["was"] > 0)), 1, 0
    )
    cps_recs["case2"] = np.where(
        ((cps_recs["filst"] == 0) & (cps_recs["was"] <= 0)), 1, 0
    )
    np.random.seed(409)
    ...
    cps_recs["z1"] = cps_recs.apply(
        lambda row: np.random.uniform(0, 1)
        if row["case1"] == 1
        else 1
        if row["case2"]
        else 0,
