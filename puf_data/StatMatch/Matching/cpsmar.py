...
def create_cps(raw_cps):
    ...
    """
    # Read in CPS file
    cps = [line.strip().split() for line in open(raw_cps).readlines()]
    ...
    for record in tqdm(cps):
        ...
        if rectype == "1":
            ...
            house_fam = pd.concat([house_rec, f_recs(record[0])], axis=1, sort=False)
        else:
            # If it's a person record, concat to household and family record
            final_rec = pd.concat([house_fam, p_recs(record[0])], axis=1, sort=False)
